...
function toHexByte(v)
  return HEX_CHARS[((v >> 4) & 0xF) + 1]..HEX_CHARS[(v & 0xF) + 1]
end
...
function formatByteFrac(hi, lo, signed)
  local word = (hi << 8) | lo
  local sign = ''
  if (hi & 0x80) ~= 0 and signed then
    word = (word ~ 0xFFFF) + 1
    sign = '-'
  end
  
  if DISPLAY_MODE == 0 then --hex values
    return sign..toHexByte(word >> 8)..'.'..toHexByte(word & 0xFF)
  elseif DISPLAY_MODE == 1 then --decimal values
    return sign..(word >> 8)..'.'..(word & 0xFF)
  else  --decimals with correct fractions
    local decimalNum = (word >> 8) + (word & 0xFF)/0x100 
...
function endFrame()
  ...
  local posX, subPosX, posY, subPosY, velX, subVelX, velY, subVelY = table.unpack(memValues)
  
  emu.drawString(TEXT_POS_X, TEXT_POS_Y, "pos x: "..formatByteFrac(posX, subPosX, false))
  emu.drawString(TEXT_POS_X, TEXT_POS_Y+8, "pos y: "..formatByteFrac(posY, subPosY, false))
  emu.drawString(TEXT_POS_X, TEXT_POS_Y+8+12, 'vel x: '..formatByteFrac(velX, subVelX, true))
  emu.drawString(TEXT_POS_X, TEXT_POS_Y+8+12+8, 'vel y: '..formatByteFrac(velY, subVelY, true))
end
